...
if [ ! -d "/home/vagrant/terramino-go/.git" ]; then
  cd /home/vagrant
  rm -rf terramino-go
#   run git with the vagrant user instead of the default root, so no errors on build
  sudo -u vagrant git clone https://github.com/hashicorp-education/terramino-go.git
  cd terramino-go
#   run git with the vagrant user instead of the default root, so no errors on build
